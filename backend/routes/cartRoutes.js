...
router.get('/', verifyToken, async (req, res) => {
    try {
        const cart = await Cart.findOne({ userId: req.user._id }).populate('items.itemId'); // Use `findOne` instead of `find`
        if (!cart) {
            return res.status(200).json({ items: [] }); // Return an empty array if the cart is empty
        }
        res.status(200).json({ items: cart.items }); // Return the items array
    } catch (err) {
